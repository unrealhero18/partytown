/* Partytown 0.6.2-dev1658833149407 - MIT builder.io */
(window => {
            ...
            if (scriptElm) {
                ...
                if (!winCtx.$isInitialized$) {
                    ...
                    ((worker, $winId$, win) => {
                        ...
                        const pushBackData = (forwardArr, args) => {
                            if (pushBacks.length > 0 && pushBacks.includes(forwardArr.join("."))) {
                                if (2 === forwardArr.length && "push" === forwardArr[1]) {
    ...
    (async receiveMessage => {
        ...
        if (onMessageHandler) {
            worker = new Worker(libPath + "partytown-ww-atomics.js?v=0.6.2-dev1658833149407", {
                name: "Partytown 🎉"
            });
            worker.onmessage = ev => {
                const msg = ev.data;
            ...
            logMain("Created Partytown web worker (0.6.2-dev1658833149407)");
